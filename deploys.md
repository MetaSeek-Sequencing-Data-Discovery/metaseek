...
check celery is happy 

`screen -r celery`

if not CTRL C to cancel, then purge the queue and restart
...
purge and rebuild the cache in a browser window:
`https://api.metaseek.cloud/cache/purge`
`https://api.metaseek.cloud/cache/build`
